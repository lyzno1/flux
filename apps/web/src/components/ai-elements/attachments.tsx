			...
			<div
				className={cn(
					...
					variant === "inline" && [
						...
						"hover:bg-accent-subtle hover:text-accent-foreground",
					],
...
export const AttachmentPreview = ({ fallbackIcon, className, ...props }: AttachmentPreviewProps) => {
	...
	const renderContent = () => {
		...
		if (mediaCategory === "video" && data.type === "file" && data.url) {
			return <video aria-label={data.filename ?? "Video"} className="size-full object-cover" muted src={data.url} />;
		}
		...
		<Button
			...
			className={cn(
				variant === "grid" && [
					...
					"opacity-0 transition-opacity focus-visible:opacity-100 group-focus-within:opacity-100 group-hover:opacity-100 motion-reduce:transition-none",
					"hover:bg-background",
					"[&>svg]:size-3",
				],
				variant === "inline" && [
