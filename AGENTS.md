...
- **Design tokens:** Do not create component-specific CSS tokens (e.g. `--cta-bg`, `--card-header-color`). Always reuse existing generic tokens (`accent`, `surface-*`, `primary`, `secondary`, etc.) unless explicitly approved by the user.
- **Type conventions:** Prefer `type` over `interface` for object type definitions. Only use `interface` when declaration merging is explicitly needed (e.g. i18next type augmentation). This prevents accidental declaration merging and is more consistent with type intersection (`&`) composition.
- **No facade hooks:** Do not create hooks that bundle unrelated store selectors and actions into a single return object. Components should import selectors and actions directly from their modules. Facade hooks force every consumer to subscribe to all state, defeating granular re-render optimization — they are barrel files for state.
